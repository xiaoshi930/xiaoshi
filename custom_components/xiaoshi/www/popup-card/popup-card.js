class PopupCard extends HTMLElement {
    ...
    setConfig(config) {
      if (!config.card) {
        throw new Error('popup_card 需要配置 card 参数');
      }
      this.config = config;
    }
    ...
    render() {
      this.shadowRoot.innerHTML = `
        ...
        ">${this.config.button_text || '打开弹窗'}</button>
      `;
      ...
      button.addEventListener('click', () => {
        window.GlobalPopupController.show({
          ...
          hide_header: this.config.hide_header
        });
      });
...
try {
  if (!customElements.get('popup-card')) {
    ...
    console.error('[popup_card] ℹ️ popup-card 已存在，跳过注册');
  }
} catch (error) {
...
if (typeof window !== 'undefined') {
  ...
  window.interceptCallService = interceptCallService;
}
