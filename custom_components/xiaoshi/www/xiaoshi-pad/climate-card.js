...
window.customCards.push({
    ...
    name: '消逝卡(A平板端)-空调/水暖毯/热水器/加湿器/除湿机卡',
  ...
  render() {
    ...
    return html`
      <div class="form">
        ...
        <!-- 定时器位置 -->
        ${this.config.enable_timer !== false ? html`
          <div class="form-group">
            <label>定时器位置</label>
            <select
...
class XiaoshiPadClimateCard extends LitElement {
  ...
  static get styles() {
    return css`      :host { display: flex; align-items: stretch; gap: 8px; }
      ...
      .history-btn { position: absolute; top: 12px; left: 16px; z-index: 10; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; cursor: default; transition: all 0.3s ease; background: transparent; }
...
_renderExtraButtons(buttonType = 1) {
  ...
  _setHvacMode(mode) {
      ...
      if (mode === 'off' && this.config.enable_timer !== false) {
          this._cancelTimer();
      }
      this._handleClick();
  }
  ...
  _getHistoryAccentColor() {
    ...
    if (this.config.entity?.startsWith('humidifier.')) {
      if (this._isDehumidifier()) return 'rgb(243,156,18)';
      return 'rgb(33,150,243)';
    }
    if (this.config.entity?.startsWith('water_heater.')) return 'rgb(254,111,33)';
  ...
  _getStateColor(state) {
    ...
    if (s === 'electric' || s === 'gas' || s === 'high_demand' || s === 'performance') return 'rgb(254,111,33)';
    return 'rgb(254,111,33)';
  }
