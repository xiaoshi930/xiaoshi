...
const editorCommonStyles = css`  .form { display: flex; flex-direction: column; gap: 10px; min-height: 500px; }
...
const cardCommonStyles = css`  :host { display: block; max-width:500px; margin: 0 auto; }
  ...
  .device-count.zero { background: rgb(250, 20, 0); }
  ...
  .devices-grid .device-item:nth-child(2) { border-top: 1px solid rgb(150,150,150,0.5); }
...
class XiaoshiConsumablesCardEditor extends ConsumablesEditorMixin(LitElement) {
  ...
  _entityChanged(e) {
    ...
    if (!value && name !== 'theme' && name !== 'width' && name !== 'popup_background') return;
...
target:
  ...
  _entityChanged(e) {
    ...
    if (type === 'checkbox') {
      ...
      if (!value && name !== 'theme' && name !== 'button_width' && name !== 'button_height' && name !== 'button_font_size' && name !== 'button_icon_size' && name !== 'popup_width' && name !== 'popup_top' && name !== 'display_mode' && name !== 'decimal_precision' && name !== 'button_text' && name !== 'popup_background' && name !== 'tap_action' && name !== 'hold_action') return;
      finalValue = value;
    }
...
class XiaoshiConsumablesButton extends ConsumablesBaseMixin(LitElement) {
  ...
  _handleButtonClick() {
    ...
    const excludedParams = ['type', 'button_height', 'button_width', 'button_font_size', 'button_icon_size', 'popup_top', 'popup_width', 'display_mode', 'decimal_precision', 'specific_entity_id', 'button_icon', 'button_text', 'transparent_bg', 'lock_white_fg', 'other_cards', 'popup_cards', 'popup', 'hold_popup_cards', 'tap_action', 'hold_action'];
