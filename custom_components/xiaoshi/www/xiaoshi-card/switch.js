...
class XiaoshiSwitchCardEditor extends LitElement {
  ...
  static get styles() {
    return css`      .form { display: flex; flex-direction: column; gap: 10px; min-height: 300px; }
      ...
      .entity-row { color: #000; display: flex; align-items: center; gap: 8px; margin-bottom: 5px; }
      .entity-row label { color: #000; min-width: 50px; font-weight: normal; font-size: 13px; }
      ...
      .add-button { border: 1px solid #c8191d; border-radius: 4px; padding: 8px; transition: all 0.2s ease; }
      .add-button:hover { background-color: rgba(200, 25, 29, 0.1); transform: translateY(-1px); box-shadow: 0 2px 4px rgba(200, 25, 29, 0.2); }`;
  }
...
class XiaoshiSwitchCard extends LitElement {
  ...
  static styles = css`    .card-container { border-radius: 16px; overflow: hidden; display: flex; flex-direction: column; padding-bottom: 10px; max-width: 500px; margin: auto; }
    ...
    .switch-count { font-size: 13px; border-radius: 8px; width: 30px; height: 30px; text-align: center; line-height: 30px; font-weight: bold; }
    ...
    .power-button.active { background: #c8191d; }
    .power-button.inactive { background: #c8191d33; pointer-events: none; }
    .power-button.unlocked { pointer-events: auto; }
    ha-icon { --mdc-icon-size: 22px; color: var(--icon-color, #666666); }
    .power-button.active.unlocked ha-icon,
    .power-button.inactive.unlocked ha-icon { color: #c8191d; }
    .power-button.active.unlocked ha-icon { color: white; }
    .lock-button { cursor: default; flex-shrink: 0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; border-radius: 6px; background: rgba(180,180,180,0.15); margin-right: 4px; }
    .lock-button ha-icon { --mdc-icon-size: 18px; transition: all 0.3s ease; color: #999; }
    .lock-button.unlocked ha-icon { color: #4CAF50 !important; }
  ...
  _updateHistoryContent() {
    ...
    for (const [entityId, data] of entries) {
      ...
      for (const { entry, time, durationMs } of filtered) {
        ...
        htmlStr += `<div style="border-radius:10px;padding: 0px 12px;margin-bottom:8px;background:${entryBg};"><div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-size:0.8rem;padding:2px 4px;border-radius:10px;font-weight:500;color:${stateColor};">${stateLabel} · ${durationStr}</span><span style="font-size:0.75rem;color:${isDark?'#aaa':'#999'};">${timeStr}</span></div></div>`;
      }
      htmlStr += `</div>`;
    }
