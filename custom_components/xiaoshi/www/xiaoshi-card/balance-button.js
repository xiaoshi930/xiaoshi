...
class XiaoshiBalanceButton extends LitElement {
  ...
  static _injectPopupStyles() {
    if (XiaoshiBalanceButton._stylesInjected) return;
    XiaoshiBalanceButton._stylesInjected = true;
  ...
  _showNativePopup(popupContent) {
    ...
    if (!hassObj) {
      console.error('[XiaoshiBalanceButton] 无法获取 hass 对象');
      return;
    }
  ...
  async _createPopupCard(container, cardConfig, hassObj) {
    try {
      ...
      console.error('[XiaoshiBalanceButton] 创建弹窗卡片失败:', err);
  ...
  _startPopupHassWatcher(hassObj) {
    ...
    try {
      ...
      console.error('[XiaoshiBalanceButton] 订阅状态变化失败:', err);
    }
  }
  ...
  _updatePopupCard() {
    if (this._popupCardElement && this._popupHass) {
      try {
        ...
        console.warn('[XiaoshiBalanceButton] 弹窗卡片更新失败:', err.message);
      }
    }
  }
