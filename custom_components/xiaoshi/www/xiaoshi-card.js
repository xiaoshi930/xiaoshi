console.info("%c 消逝卡-汇总卡 \n%c  2026.6.4.08 ", "color: red; font-weight: bold; background: black", "color: white; font-weight: bold; background: black");

import('./function/function.js');
import('./xiaoshi/phone-card.js');
import('./xiaoshi/avatar-card.js');
import('./xiaoshi/dynamic-card.js');
import('./xiaoshi/room-card.js');
import('./xiaoshi/popup-card.js')

import('./xiaoshi-card/ha-info.js'); 
import('./xiaoshi-card/todo.js');
import('./xiaoshi-card/chart.js');
import('./xiaoshi-card/balance.js');
import('./xiaoshi-card/consumables.js');
import('./xiaoshi-card/petrochina.js');
import('./xiaoshi-card/state-grid.js');
import('./xiaoshi-card/weather.js');
import('./xiaoshi-card/button.js');
import('./xiaoshi-card/device.js');
import('./xiaoshi-card/light.js');
import('./xiaoshi-card/computer-card.js');
import('./xiaoshi-card/pve-card.js');
import('./xiaoshi-card/class-card.js');

import('./xiaoshi-phone/climate-card.js');
import('./xiaoshi-phone/humidifier-card.js');
import('./xiaoshi-phone/purifier-card.js');
import('./xiaoshi-phone/computer-card.js');
import('./xiaoshi-phone/light-card.js');
import('./xiaoshi-phone/switch-card.js');
import('./xiaoshi-phone/music-player-card.js');
import('./xiaoshi-phone/tv-player-card.js');
import('./xiaoshi-phone/fan-card.js');
import('./xiaoshi-phone/cover-card.js');
import('./xiaoshi-phone/other-card.js');

import('./xiaoshi-small/climate-card.js');
import('./xiaoshi-small/humidifier-card.js');
import('./xiaoshi-small/purifier-card.js');

import('./xiaoshi-pad/pad-card.js');
import('./xiaoshi-pad/avatar-card.js');
import('./xiaoshi-pad/dynamic-card.js');
import('./xiaoshi-pad/top-bar-card.js');

import('./xiaoshi-pad/chinese-poetry-card.js');
import('./xiaoshi-pad/climate-card.js');
import('./xiaoshi-pad/fan-card.js');
import('./xiaoshi-pad/grid-card.js');
import('./xiaoshi-pad/weather-card.js');
import('./xiaoshi-pad/sun-moon-card.js');

import('./xiaoshi-lunar/birthday-card.js');
import('./xiaoshi-lunar/lunar-calendar.js');
import('./xiaoshi-lunar/lunar-calendar-pad.js');
import('./xiaoshi-lunar/lunar-calendar-phone.js');
